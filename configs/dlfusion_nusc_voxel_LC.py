...
data = dict(
    samples_per_gpu=1,
    workers_per_gpu=10,
    samples_per_epoch=30000,  # The number of training samples per epoch
    train=dict(
        type='CBGSDataset',
        dataset=dict(
            type=dataset_type,
...
model = dict(
    ...
    imgvoxel_lidar_fusion=dict(
        ...
        img_voxel_fuser=dict(
            type='IVFuser',
            in_channels=256,
            out_channels=80,
        ),
        embed_dims_for_fuser=80,
        collapse_pts=False,
        collapse_pts_in=400,
        collapse_pts_out=256,
    ),

    # imgvoxel_encoder=dict(
    #     type='SelfResNet3D',
    #     depth=18,
    #     block_inplanes=[64, 128, 128, 256],
...
log_level = 'INFO'
work_dir = None
load_from = "/mnt/data/exps/TransFusion-master/work_dirs/dlfusion_nusc_voxel_L/latest.pth"
resume_from = None
workflow = [('train', 1)]
gpu_ids = range(0, 8)
