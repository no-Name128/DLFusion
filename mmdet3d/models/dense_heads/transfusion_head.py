...
from ..utils import img_voxel_proj
...
class TransFusionHead(nn.Module):
    ...
    def create_2D_grid(self, x_size, y_size):
        ...
        batch_x = batch_x + 0.5
        batch_y = batch_y + 0.5
        coord_base = torch.cat([batch_x[None], batch_y[None]], dim=0)[None] # 2 x y
        coord_base = coord_base.view(1, 2, -1).permute(0, 2, 1)  # 2 xy -> 1 2 xy -> 1 xy 2
        return coord_base
    ...
    def forward_single(self, inputs, img_inputs, img_metas):
        ...
        if self.initialize_by_heatmap:
            ...
            top_proposals = heatmap.view(batch_size, -1).argsort(dim=-1, descending=True)[..., :self.num_proposals] # b top
            top_proposals_class = top_proposals // heatmap.shape[-1]  # 上一步转换伪了  b chw 的数据，得到了排名前 top 的数据，在这里使用  //  对前top个位置（带有类别）的数进行归类，看看它属于哪一个类别，理解为  chw // hw 近似等于 c
            top_proposals_index = top_proposals % heatmap.shape[-1] # chw % hw 近似理解为 得到该数据的位置，用于检索 # b 128
            query_feat = lidar_feat_flatten.gather(index=top_proposals_index[:, None, :].expand(-1, lidar_feat_flatten.shape[1], -1), dim=-1) # b 10 128
            ...
            query_pos = bev_pos.gather(index=top_proposals_index[:, None, :].permute(0, 2, 1).expand(-1, -1, bev_pos.shape[-1]), dim=1) # bev_pos:b hw 2  ,gather operation "dim" for the dim of "hw" ; query_pos: b 128 2
        else:
        ...
        for i in range(self.num_decoder_layers):
            ...
            res_layer['center'] = res_layer['center'] + query_pos.permute(0, 2, 1)  # b 2 128
            first_res_layer = res_layer
            if not self.fuse_img:
                ret_dicts.append(res_layer)

            # for next level positional embedding
            query_pos = res_layer['center'].detach().clone().permute(0, 2, 1) # b 128 2
        ...
        if self.fuse_img:
            ...
            query_pos_realmetric = query_pos.permute(0, 2, 1) * self.test_cfg['out_size_factor'] * self.test_cfg['voxel_size'][0] + self.test_cfg['pc_range'][0] # b 2 128
            query_pos_3d = torch.cat([query_pos_realmetric, res_layer['height']], dim=1).detach().clone() # b 2 128 (cat) b 1 128  -> b 3 128
            if 'vel' in res_layer:
            ...
            for sample_idx in range(batch_size if self.fuse_img else 0):
                ...
                for view_idx in range(self.num_views):
                    ...
                    coor_x, coor_corner_x = coor_x[0:self.num_proposals, :], coor_x[self.num_proposals:, :] #  # [3, num_proposals] + [3, num_proposals*8]
                    ...
                    coor_corner_xy = torch.cat([coor_corner_x, coor_corner_y], dim=-1) # 128 8 2
                    ...
                    center_ys = (coor_y[on_the_image] / self.out_size_factor_img) # 128 1 
                    center_xs = (coor_x[on_the_image] / self.out_size_factor_img) # 128 1  
                    centers = torch.cat([center_xs, center_ys], dim=-1).int()  # center on the feature map  # 128 2
                    ...
                    distance = (centers[:, None, :] - (img_feat_pos - 0.5)).norm(dim=-1) ** 2  # (128 1 2 - 1 hw 2)  # 128 hw 2 -> norm(dim=-1) -> 128 hw
                    gaussian_mask = (-distance / (2 * sigma[:, None] ** 2)).exp()
                    gaussian_mask[gaussian_mask < torch.finfo(torch.float32).eps] = 0
                    attn_mask = gaussian_mask  # atten mask size is equal to the size of q@k.T ,and then is equal to the scores(qkv) of multiheadattention

                    query_feat_view = prev_query_feat[sample_idx, :, on_the_image]
                    query_pos_view = torch.cat([center_xs, center_ys], dim=-1) # 1 128 2
