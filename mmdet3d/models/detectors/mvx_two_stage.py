...
class MVXTwoStageDetector(Base3DDetector):
    ...
    def __init__(self,
                 ...
                 train_cfg=None,
                 test_cfg=None,
                 imgvoxel_lidar_fusion=None,
                 imgvoxel_encoder=None,
                 imgvoxel_fpn3d=None,
                 pretrained=None,
                 pretrained_img=None,
                 pretrained_pts=None,
        ...
        self.pts_neck_cfg = pts_neck
        if pts_neck is not None:
            self.pts_neck = builder.build_neck(pts_neck)
        self.pts_bbox_head_cfg = pts_bbox_head
        if pts_bbox_head:
        ...
        # 构建将图像2D体素特征提升到三维空间和LiDAR三维体素空间融合的模块，以代替原有的LiDAR体素三维特征
        self.imgvoxel_lidar_fusion_cfg = imgvoxel_lidar_fusion
        if imgvoxel_lidar_fusion:
            self.imgvoxel_lidar_fusion = builder.build_fusion_layer(imgvoxel_lidar_fusion)
            
        if imgvoxel_encoder:
            self.imgvoxel_encoder=builder.build_backbone(imgvoxel_encoder)
            
        if imgvoxel_fpn3d:
            self.imgvoxel_fpn3d=builder.build_neck(imgvoxel_fpn3d)
    ...
    def extract_feat(self, points, img, img_metas):
        ...
        if self.with_img_backbone:
            if self.imgvoxel_lidar_fusion_cfg:
                aug_pts_feats = self.imgvoxel_lidar_fusion(img, img_feats, pts_feats, img_metas)

                if self.imgvoxel_lidar_fusion.collapse_pts:
                    _, pts_feats = torch.max(aug_pts_feats, 2)
                    pts_feats = pts_feats.type(torch.cuda.FloatTensor)
                else:
                    b, c, d, h, w = aug_pts_feats.shape
                    aug_pts_feats = aug_pts_feats.view(b, c * d, h, w)
                    pts_feats = self.imgvoxel_lidar_fusion.channel_reduce(aug_pts_feats)
                    pts_feats = pts_feats.type(torch.cuda.FloatTensor)
